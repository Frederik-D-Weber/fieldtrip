...
elseif filetype_check_extension(filename, '.edf') && ...
        exist(fullfile(p, ['EEG L' '.edf']), 'file') && ...
        exist(fullfile(p, ['EEG R' '.edf']), 'file') && ...
        true
    % the following channels are possible files, and likely all are expected to
    % exist, however the 'EEG L' and 'EEG R' are considered essential as well
    % as identifiyng for this data format.
    %         exist(fullfile(p, ['BATT' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['BODY TEMP' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['dX' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['dY' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['dZ' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['EEG L' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['EEG R' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['LIGHT' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['NASAL L' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['NASAL R' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['NOISE' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_DARK_AC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_DARK_DC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_IR_AC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_IR_DC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_R_AC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['OXY_R_DC' '.edf']), 'file') && ...
    %         exist(fullfile(p, ['RSSI' '.edf']), 'file') && ...

  type = 'zmax_edf';
  manufacturer = 'Hypnodyne Corp.';
  content = 'continuous PSG data';
